...
class CoinDeadboltHandler(coinLogic: CoinService)(implicit ec: ExecutionContext)
  ...
  private def getCoinFromHeaders(request: RequestHeader): Try[CoinCode] = {
    request.headers.get(COIN_PARAM) match {
      case Some(code) ⇒ CoinCode.fromString(code)
